# Dacebook - Simple Social App (PHP Native)

**Dacebook** is a simple web application built using **HTML**, **CSS**, **Bootstrap**, and **PHP Native**. This project includes basic user authentication (login & signup) and CRUD functionality for posts.

## 🔧 Features

- ✅ User registration (Sign up)
- ✅ User login & logout
- ✅ Create, read, update, delete (CRUD) posts
- ✅ Responsive design with Bootstrap
- ✅ PHP Native (no framework)

## 💻 Technologies Used

- HTML & CSS
- Bootstrap 5
- PHP (Native)
- MySQL

## ⚠️ Disclaimer

This project is for learning purposes only and may lack security features required for production use (e.g., input sanitization, password hashing, etc.).

---

## 📬 Contact

Feel free to reach out if you have feedback or questions:

- GitHub: [michaelandrea123](https://github.com/michaelandrea123)
